...
import { UserCheck, Search, Shield, Save } from 'lucide-react';
...
export default function AssignClientsPage() {
  ...
  const columns = [
    ...
    },
    {
      ...
      cell: ({ row }) => <span className="font-bold text-xs capitalize text-slate-900 dark:text-white">{row.original.CustomerName || row.original.CID}</span>,
    },
  ];
