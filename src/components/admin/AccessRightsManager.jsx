...
export function AccessRightsManager() {
  ...
  useEffect(() => {
    Promise.all([loadSubUsersList(), loadAccessSummary()]);
  }, []);
  ...
  const columns = [
    {
      ...
      cell: ({ row }) => <span className="font-bold text-xs capitalize text-slate-900">{row.original.StaffName || row.original.ARfSatffID}</span>,
    },
    {
      header: 'Contacts',
    ...
    <div className="space-y-6">
      <Card>
        ...
        <CardContent className="space-y-6">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4 p-4 rounded-xl border border-slate-200 bg-slate-50">
            {[
